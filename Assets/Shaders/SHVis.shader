Shader "Custom/SHVis"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            real SHEvalLinearL2(real3 N, real4 shB, real4 shC)
            {
                real x2;
            ...
            float SampleSH9(float4 SHCoefficients[3], float3 N)
            {
                float4 shA = SHCoefficients[0];
                float4 shB = SHCoefficients[1];
                float4 shC = SHCoefficients[2];

                // Linear + constant polynomial terms
                float res = SHEvalLinearL0L1(N, shA);

                // Quadratic polynomials
                res += SHEvalLinearL2(N, shB, shC);
            ...
            void Pixel(PackedVaryingsToPS packedInput, out float4 outColor : SV_Target0)
            {
                ...
                half3 ambient_contrib = SampleSH(bsdfData.normalWS);
                outColor = half4(ambient_contrib, 1);
                // outColor = half4(bsdfData.normalWS,1);
                // outColor = half4(1,1,0,0.5);
            }
